...
declare namespace ICCreditComponents.Constantes.Inject {
    ...
    namespace Services {
        ...
        const multiInstanceWorkerService: string;
...
declare namespace ICCreditComponents.Directives.RedacteurActe {
    ...
    class RedacteurActeDirectiveService implements IRedacteurActeDirectiveService {
        ...
        static $inject: string[];
        constructor(redacteurActeService: ICBusiness.Services.RedacteurActe.IRedacteurActeService, $q: ng.IQService, mwNotificationService: MyWay.UI.IMwNotificationService);
        saveInfoDirectiveIfNeeded(redacteurActeViewModel: IRedacteurActeViewModel): ng.IPromise<void>;
        private definirRedacteurActeAndPrintAno(redacteurActeViewModel);
        needsSaving(redacteurActeViewModel: IRedacteurActeViewModel): boolean;
...
declare namespace ICCreditComponents.Directives.RedacteurActe {
    ...
    interface IRedacteurActeViewModel {
        ...
        redacteurActeEmailPopoverContent: string;
    }
...
declare namespace ICCreditComponents.Directives.ReponseAssureur {
    ...
    class ReponseAssureurControleur {
        ...
        private code5;
        private code6;
        private codeEtatAccessoireInitial;
        ...
        optionsQS: ICBusiness.Models.Common.ICodeLabel<boolean>[];
        static $inject: string[];
...
declare namespace ICCreditComponents.Directives.SyntheseFinanciere {
    import BusinessModel = ICBusiness.Models;
    interface IParamsSyntheseFinanciere {
        estAvantProjet: boolean;
        creditsHistogramme: ICBusiness.Models.Financement.IHistogrammeSynthese;
    }
    interface IBulleId {
        chargeIdInfoBulle: string;
        chargeOctroiIdInfoBulle?: string;
        ravIdInfoBulle: string;
        tauxEffortIdInfoBulle: string;
    }
    class SyntheseFinanciereControleur {
        ...
        codeDevise: string;
        params: IParamsSyntheseFinanciere;
        bulleIds: IBulleId;
        libelleChargeAnnuelle: string;
        static $inject: string[];
        constructor($scope: ng.IScope, $filter: ng.IFilterService, chartService: Services.IChartService);
        libelleGraphique: string;
        private formatDate(myDate);
...
declare namespace ICCreditComponents.Services {
    ...
    class ApplicationService implements IApplicationService {
        ...
        constructor($q: ng.IQService, dossierService: ICBusiness.Services.Dossier.IDossierService, popinService: Services.IPopinService, repriseDossierService: Services.IMultiInstanceWorkerService, traceService: ICCreditComponents.Services.ITraceService, controleSyntheseService: ICCreditComponents.Controle.IControleSyntheseService, loaderService: ICCreditComponents.Services.ILoaderService);
        fermerApplication(context: ICBusiness.Models.Common.IContext, navigationService: Navigation.INavigationCreditService, sauvegarde?: boolean): void;
        private afficherPopinErreurDossier(message, identifiantDossier, captionOk?, captionAnnuler?);
        private fermerDossier(identifiantDossier, context, sauvegarde?);
        private LancerFermetureDossier(identifiantDossier, sauvegarde?);
        switchToConsultationMode(appParams: ICBusiness.Models.Common.ILowLevelParams, navigationService: ICCreditComponents.Navigation.INavigationUtils, fermerDossier?: boolean): ng.IPromise<void>;
    }
}

declare namespace ICCreditComponents.Services {
    namespace ChartsModel {
        const colors: string[];
    ...
    interface IChartService {
        drawArcGraphic(document: Document, containerId: string, elementId: string, data: ChartsModel.IBaseData[]): void;
        drawHistogramGraphic(document: Document, elementId: string, data: ChartsModel.ISerieData[], margins?: {
            t: number;
            l: number;
            b: number;
            r: number;
    ...
    class ChartService implements IChartService {
        ...
        drawArcGraphic(document: Document, containerId: string, elementId: string, data: ChartsModel.IBaseData[]): void;
        drawHistogramGraphic(document: Document, elementId: string, dataSeries: ChartsModel.ISerieData[], margins?: {
            t: number;
            l: number;
            b: number;
            r: number;
        ...
        private mapperDonnees(data);
        private getYStart(yScale, d, middle?);
        private getYEnd(yScale, yMax, d);
        private getTextBaseData(d);
        private getTextBarData(yScale, d);
        private getTitleBarData(yScale, d);
        private isSmallBarData(yScale, d, maxPixel?);
        private dessinerGraphique(svg, series, yMax, yScale, xScale, getLegendSerie);
    }
}
...
declare namespace ICCreditComponents.Services {
    ...
    class EditionOffreService implements IEditionOffreService {
         ...
         * @param data données d'impression
         */
        private traitementRetourEditionSiAuMoinsUnDocument(data, doc);
        /**
         ...
         * @param nombreTotalImpression Nombre total d'impression à envoyer au suivi des impressions
         */
        private alimenteListeDocuments(data, nombreTotalImpression);
        /**
...
declare namespace ICCreditComponents.Services {
    interface IInfoInstanceDossier {
        processId: string;
        idDossier: string;
        codeEtablissement: string;
        referenceExterneAgent: string;
        dateTraitement: string;
        codeApplication: ICBusiness.Enum.CodeApplicationEnum;
        repriseDossier: boolean;
        modeOuverture: ICBusiness.Enum.ActionOuvertureDossierEnum;
        refusReprise: boolean;
    }
    interface IStatutInstance {
        reprendreSauvegardeLocal: boolean;
        isOpenSomewhereElse: boolean;
    }
    interface IMultiInstanceWorkerService {
        checkDossierInstances(idDossier: string): IStatutInstance;
        checkInstanceAlive(idDossier: string): void;
        setWorkingInstance(idDossier: string, codeEtablissement: string, referenceExterneAgent: string, dateTraitement: string, codeApplication: ICBusiness.Enum.CodeApplicationEnum, modeOuverture: ICBusiness.Enum.ActionOuvertureDossierEnum): void;
        closeInstance(idDossier: string): void;
        setUpDependencies(navigationService: Navigation.INavigationCreditService): void;
    }
    class MultiInstanceWorkerService implements IMultiInstanceWorkerService {
        private serviceAgent;
        private $interval;
        private $location;
        private popinService;
        ...
        static $inject: string[];
        constructor(serviceAgent: MyWay.Services.ServiceAgentExtended, $interval: ng.IIntervalService, $location: ng.ILocationService, popinService: ICCreditComponents.Services.IPopinService);
        setUpDependencies(navigationService: Navigation.INavigationCreditService): void;
        checkDossierInstances(idDossier: string): IStatutInstance;
        private intervalPromise;
        checkInstanceAlive(idDossier: string): void;
        closeInstance(idDossier: string): void;
        private isInstanceAlive(idDossier);
        setWorkingInstance(idDossier: string, codeEtablissement: string, referenceExterneAgent: string, dateTraitement: string, codeApplication: ICBusiness.Enum.CodeApplicationEnum, modeOuverture: ICBusiness.Enum.ActionOuvertureDossierEnum): void;
        private getCookieName(idDossier);
        private getCookie(cname);
        private setCookie(cname, cvalue);
        private removeCookie(cname);
    }
}
...
declare namespace ICCreditComponents.Services {
    interface IPrendreDecisionPreteur {
         ...
         */
        controleAvantDecision(context: ICBusiness.Models.Common.IContext, dateAccordPreteur: string): ng.IPromise<ICBusiness.Models.Dossier.IControleDossier>;
...
declare namespace ICCreditComponents.Services {
    interface IVerifierEligibiliteService {
        openModaleVerifierEligibilite(identifiantDossier: string, identifiantPlan: string, coutTotal: number): void;
    }
...
declare namespace ICCreditComponents.Controle {
    class SyntheseAnomaliesDirectiveControleur {
        ...
        hasModalLink(ano: ICBusiness.Models.Dossier.IAnomalie): boolean;
        ...
        goToModale(pageId: string, anomalie: ICBusiness.Models.Dossier.IAnomalie): void;
        rafraichir(): void;
    }
}
...
declare namespace ICCreditComponents.Controle {
    interface IControleSyntheseService {
        ...
        controlerAvantEditionOffre(idDossier: string, premierResultat?: ICBusiness.Models.Edition.IAnomaliesControleEdition): ng.IPromise<ICBusiness.Models.Edition.IAnomaliesControleEdition>;
        onControlerCompletudeEtDecisionDone(eventHandler: () => void): void;
        redirect(pageid: string): void;
        redirectAnomalie(pageid: string, anomalie: ICBusiness.Models.Dossier.IAnomalie): void;
        /**
         * Fournit une fonction à appeler avant de rediriger vers l'anomalie (ex : on est déjà sur la page, donc on veut ouvrir la popin sans redirection)
         * @param delegate fonction appelée qui renvoie vrai si on continue la redirection, faux si on souhaite l'interrompre
         */
        onBeforeRedirectAnomalie(delegate: () => ng.IPromise<boolean>): void;
        /**
        ...
        /** Retourne la dernière anomalie cliquée (lien) s'il y en a */
        getAnomalieRedirection(): ICBusiness.Models.Dossier.IRedirectionAnomalieControle;
        /** Nettoie le stockage de la dernière anomalie cliquée */
        clearAnomalieRedirection(): void;
        traiterControleEdition(resultatControle: ICBusiness.Models.Edition.IAnomaliesControleEdition): void;
    }
    class ControleSyntheseService {
        ...
        private mwNotificationService;
        private $rootScope;
        private _anomaliePourRedirection;
        private _onBeforeRedirectAnomalie;
        nombreAnomalieBloquante: number;
        nombreAlertes: number;
        ...
        controleEffectue: boolean;
        static $inject: string[];
        constructor($q: ng.IQService, $timeout: ng.ITimeoutService, controleDossierService: ICBusiness.Services.Dossier.IControleDossierService, editionOffreService: ICBusiness.Services.Edition.IEditionOffreService, anomalieService: Services.IAnomalieService, labelService: Services.IlabelService, contextService: ICCommon.Services.IProcessusContextService, mwNotificationService: MyWay.UI.IMwNotificationService, $rootScope: ng.IScope);
        /**
         ...
         * @param pageid
         */
        redirect(pageid: string, typeObjetAnomalie?: ICBusiness.Enum.TypeObjetAnomalieEnum): void;
        redirectAnomalie(pageId: string, anomalie: ICBusiness.Models.Dossier.IAnomalie): void;
        private setAnomalieRedirection(anomalie);
        clearAnomalieRedirection(): void;
        getAnomalieRedirection(): ICBusiness.Models.Dossier.IRedirectionAnomalieControle;
        resetControle(): void;
        /**
        ...
        controlerCompletude(idDossier: string): ng.IPromise<ICBusiness.Models.Dossier.IControleDossier>;
        /**
         * Si on appelle depuis editionOffreService, on a déjà lancer le contrôle donc on passe les résultats directement
         * juste pour traiter l'affichage des totems et le rafraichissement sur changement de SPA.
         * Sinon on est dans le cas d'un changement de SPA et on veut rafraichir les contrôles pour mettre à jour les totems
         * @param idDossier
         * @param premierResultat
         */
        controlerAvantEditionOffre(idDossier: string, premierResultat?: ICBusiness.Models.Edition.IAnomaliesControleEdition): ng.IPromise<ICBusiness.Models.Edition.IAnomaliesControleEdition>;
        /**
         * Controle dossier pour passer l'accord client
        ...
        /** On distingue les 2 sauvegardes car elles sont indépendantes mais stockée sous la même clef (contexte lisa) */
        private setSauvegardeAnomalieRedirection();
        ...
        /**
         * Transforme le retour du contrôle édition (spécifique) en IControleDossier pour uniformiser les traitements
         * @param resultatControle
         */
        private mapToControlResult(resultatControle);
    }
}
...
declare namespace ICCreditComponents.Directives {
    class HeaderV2DirectiveControleur {
        private contextEditiqueService;
        private popinService;
        private totemV2Service;
        static parametres: ICCommon.Controleur.IControleurParametre;
        onCloseClick: () => void;
        context: ICBusiness.Models.Common.IContext;
        voletContextEditionOuvert: boolean;
        static $inject: string[];
        constructor(contextEditiqueService: ContextEdition.IContexteEditionService, popinService: Services.IPopinService, totemV2Service: Navigation.TotemV2Service);
        infosDossier: ICBusiness.Models.Dossier.IDossierSchema;
        showCheminDeFer: () => boolean;
        title: string;
        /** Construction du libelle de détail sous le header */
        libelleDetail: string;
        private isLocalHost(hostname?);
        private nomCompletEmprunteur;
        private labelDossier;
        private labelDossierGestion;
        private transmisExpert;
        afficherCheminDeFer: boolean;
        afficherSyntheseAction(): boolean;
        openContexteEdition(): void;
        onClose(): void;
        onPersoClick(): void;
        onBlocNotesClick(): void;
        onHistoClick(): void;
        onSyntheseClick(): void;
        isPreferenceUtilisateurVisible: boolean;
        isBlocNotesVisible: boolean;
        aNotesDossier: number;
        isBlocNotesPage(): boolean;
...
declare namespace ICCreditComponents.Navigation {
    import IDossierSchema = ICBusiness.Models.Dossier.IDossierSchema;
    import TotemItem = ICCreditComponents.Navigation.TotemV2.ITotemItemV2;
    interface ISectionObjet {
        sectionId: string;
        el: HTMLElement;
    }
    class TotemV2DirectiveControleur {
        private $timeout;
        private totemV2Service;
        private $scope;
        static parametres: ICCommon.Controleur.IControleurParametre;
        infosDossier: IDossierSchema;
        currentPage: string;
        private _previousCurrentPage;
        private _previousSousEtat;
        private totemItemsStatic;
        private totemItemsDynamic;
        private previousSectionActive;
        static $inject: string[];
        constructor($timeout: ng.ITimeoutService, totemV2Service: Navigation.TotemV2Service, $scope: ng.IScope);
        private _debounceSetActiveSection;
        private setActiveSection(idActiveSection?);
        private _debounceSetTotem;
        private setTotems(sousEtat);
        private setSelectedTotem(totemItem);
        private defineTotemDynamic(etatDossier);
        private defineTotemStatic(etatDossier);
        totemVisible(totem: TotemItem): boolean;
    }
}

declare namespace ICCreditComponents.Navigation.TotemV2 {
    interface ITotemItemV2 {
        name: string;
        route?: string;
        sectionId?: string;
        children?: ITotemItemV2[];
        hasAno?: () => boolean;
        isCompleted?: () => boolean;
        selected?: boolean;
        visible?: () => boolean;
    ...
    interface ITotemV2ItemDirective extends ng.IScope {
        item: ITotemItemV2;
        autoExpand: boolean;
    }
}
...
declare namespace ICCreditComponents.Navigation {
    ...
    class TotemV2Service {
        ...
        private _idActiveSection;
        private context;
        private preferencesUtilisateur;
        private loadPreferenceOnce;
        static $inject: string[];
        constructor(preferenceService: ICBusiness.Services.Preferences.IPreferencesService, etatDossierService: ICBusiness.Services.Dossier.IEtatDossierService, paramApplicatifRemanentService: ICBusiness.Services.ParametrageApplicatif.IParametrageAppicatifRemanentService, suiviEditionsService: ICBusiness.Services.SuiviEditions.ISuiviEditionsService, controleSyntheseService: Controle.IControleSyntheseService, $rootScope: ng.IRootScopeService);
        setActiveSection(sectionId?: string): void;
        idActiveSection: string;
        /**
         * Mise en place des dépendances
         * @param navigationService
         * @param context
        ...
        goToBlocNoteInHeader(): void;
        goToHistoriqueInHeader(): void;
        goToSyntheseInHeader(): void;
        getTotemDynamicProposition(): TotemItem[];
        getTotemDynamicMontage(): TotemItem[];
        getTotemDynamicEdition(): TotemItem[];
        getTotemDynamicRemise(): TotemItem[];
        getTotemDynamicOffreEnAttenteSignature(): TotemItem[];
        getTotemDynamicOffreConclue(): TotemItem[];
        geTotemStaticProposition(): TotemItem[];
        geTotemStaticSignature(): TotemItem[];
        geTotemStaticOffreConclue(): TotemItem[];
        private getTotemClient();
        private getTotemProjet();
        private getTotemFinancement();
        private getVisibilityPageFinancement();
        private getTotemPretReglemente();
        private getTotemGestionAdmin();
        private getTotemPropositionRetenue();
        private getTotemDecisionPreteur();
        private getTotemEditionOffre();
        private getTotemMajAdmin();
        private getTotemContractualisation();
        private getTotemRemiseOffre();
        private getTotemSignatureOffre();
        private getTotemSignataire();
        private getTotemPieco();
        private isDefi();
        private getTotemMrh();
        private getTotemEditionProposition();
        private getTotemSectionDocumentsClients();
        private getTotemSectionRefusAssuranceExterne();
        private displayRefusAssuranceExterne();
        private getTotemEdition();
...
declare namespace ICCreditComponents.Navigation {
    interface INavigationCreditService extends ICCreditComponents.Navigation.INavigationUtils {
        ...
        goToClientModalCreditExterne(): void;
        goToProjet(navInterne?: boolean): void;
        goToProjetSectionDescription(navInterne?: boolean): void;
        goToProjetSectionMontant(navInterne?: boolean): void;
        goToProjetSectionObjetGarantie(navInterne?: boolean): void;
        goToProjetSectionGarants(navInterne?: boolean): void;
        goToProjetSectionExpertise(navInterne?: boolean): void;
        goToProjetSectionReamenagement?(navInterne?: boolean): void;
        goToProjetSectionObjetFinance?(navInterne?: boolean): void;
        ...
        goToGestionAdminSectionAvisSLAB(): void;
        ...
        goToMultiRisqueHabitation(navInterne?: boolean): void;
        goToHistorique(navInterne?: boolean): void;
        ...
        fermerApplication(close: () => ng.IPromise<boolean>): void;
...
declare namespace ICCreditComponents.Navigation {
    interface INavigationCreditCommonService {
        navigateToPageCode(pageCode: ICBusiness.Enum.CodePageEnum, navigationService: INavigationCreditService, navInterne?: boolean): void;
    }
...
declare namespace ICCreditComponents.Navigation {
    ...
    abstract class NavigationUtils {
        ...
        protected popinService: Services.IPopinService;
        protected loaderService: Services.ILoaderService;
        protected $timeout: ng.ITimeoutService;
        protected totemCreditService: ICCreditComponents.Navigation.TotemCreditService;
        protected totemV2Service: ICCreditComponents.Navigation.TotemV2Service;
        ...
        constructor($q: ng.IQService, popinService: Services.IPopinService, loaderService: Services.ILoaderService, $timeout: ng.ITimeoutService, totemCreditService: ICCreditComponents.Navigation.TotemCreditService, totemV2Service: ICCreditComponents.Navigation.TotemV2Service);
        ...
        protected setActiveSection(idSection: string): void;
    }
}
...
declare namespace ICCreditComponents.Navigation {
    class TotemCreditService {
        ...
        static $inject: string[];
        constructor($timeout: ng.ITimeoutService, labelService: Services.IlabelService, etatDossierService: ICBusiness.Services.Dossier.IEtatDossierService, controleSyntheseService: Controle.IControleSyntheseService, paramApplicatifRemanentService: ICBusiness.Services.ParametrageApplicatif.IParametrageAppicatifRemanentService, loggerService: ICCommon.Logger.FullLoggerService, preferenceService: ICBusiness.Services.Preferences.IPreferencesService, suiviEditionsService: ICBusiness.Services.SuiviEditions.ISuiviEditionsService, habilitationService: ICBusiness.Services.Habilitation.IHabilitationService, queryMultiCallService: ICCommon.Services.IRetryRequestService, $rootScope: ng.IRootScopeService);
        /**
         * Mise en place des dépendances
         * @param navigationService
         * @param context
        ...
        private afficherSectionSynthese();
...
declare namespace ICCreditComponents.Directives.RedacteurActe {
    interface IBaseRedacteurActeFormSettings {
        ...
        inputEmail: Forms.IInputSettings;
    }
...
declare namespace ICCreditComponents.Services {
    interface IFooterCreditService {
        ...
        accederDevisMRHEnabled(delegate: () => boolean): void;
        ...
        verifierEligibilitePlanGetter(delegate: () => string): void;
        calculerPropositionVisible(delegate: () => boolean): void;
        calculerPropositionEnable(delegate: () => boolean): void;
        calculerPropositionAction(delegate: () => ng.IPromise<void>): void;
    ...
    class FooterCreditService implements IFooterCreditService {
        ...
        private _accederDevisMRHEnabled;
        ...
        private _calculerPropositionVisible;
        private _calculerPropositionEnable;
        private _calculerPropositionAction;
        ...
        private _verifierEligibilitePlanGetter;
        ...
        calculerPropositionEnable(delegate: () => boolean): void;
        calculerPropositionAction(delegate: () => ng.IPromise<void>): void;
        private getButtonCalculerPropositions();
        /**
...
declare namespace ICCreditComponents.Directives.Garantie {
    ...
    class DetailGarantieAdminControleur {
        ...
        personnesListe: ICBusiness.Models.Common.ICodeLabel<string>[];
        personneSelectionnee: ICBusiness.Models.Common.ICodeLabel<string>;
        loadingAppartenantList: boolean;
        dataAjouterGarant: MyWay.UI.IBoutonActionRiche[];
        loading: boolean;
        maxLengthIdExterne: string;
        private code0;
        private code1;
        private code2;
...
declare namespace ICCreditComponents.Directives.Garantie {
    interface IDetailGarantieFormSettings {
        ...
        inputListBoxAppartenant: ICCreditComponents.Forms.IInputSettings;
    }
    interface IGarantieReponseOrganismeSettings {
        ...
        textAlertCEGC: ICCreditComponents.Forms.IInputSettings;
    }
...
declare namespace ICCreditComponents.Modales {
    ...
    interface IBien extends ICBusiness.Models.Edition.IBien {
        id: string;
        expanded: boolean;
        selected: boolean;
        rules: IFCM17GBMBienRules;
    }
    ...
    interface IFCM17GBMBienRules {
        nature: ICBusiness.Rules.IValidationRule[];
        quantite: ICBusiness.Rules.IValidationRule[];
        valeur: ICBusiness.Rules.IValidationRule[];
        lieuSituation: ICBusiness.Rules.IValidationRule[];
        allRules: ICBusiness.Rules.IValidationRule[];
    }
    interface IFCM17GBMRules {
        nomenclature: ICBusiness.Rules.IValidationRule[];
        nombreBiens: ICBusiness.Rules.IValidationRule[];
        allRules: ICBusiness.Rules.IValidationRule[];
    }
    class ModalDonneesCompNecessaireControleur {
        ...
        donneesFCM17GBM: ICBusiness.Models.Edition.IFCM17GBM;
        rulesFCM17GBM: IFCM17GBMRules;
        idDossier: string;
        title: string;
        estAvecBtnAnnuler: boolean;
        ...
        getTextAreaMaxLength(donneeComplementaire: IDonneeComplementaire): number;
        getRules(donneeComp: ICBusiness.Models.Edition.IDonneeComplementaire): ICBusiness.Rules.IValidationRule[];
        private isRulesValid();
        private getCurrentRules();
        ajouterBienInfo: string;
        /**
        ...
        private initDocumentFCM17GBM();
        private getRulesFCM17OME(outil);
        private getRulesFCM17GBMBien(bien);
        private getRulesFCM17GBM();
        estDocumentFCM17OME: boolean;
        estDocumentFCM17GBM: boolean;
        isAjoutOutilPossible: boolean;
        isAjoutBienPossible: boolean;
        private isSaisieOutilValide(outil);
        private isSaisieBienValide(bien);
        ajouterMateriel(): void;
        ajouterBien(): void;
        private compteurOutil;
        private duplicateTableRowModel();
        private compteurBien;
        private duplicateTableRowModelBien();
        private selectAndExpandTableRow(tableRow, collapseAll?, event?);
        private selectAndExpandTableRowBien(tableRow, collapseAll?, event?);
        validerMateriel(outil: IOutil): void;
        validerBien(bien: IBien): void;
        supprimerMateriel(outil: IOutil): void;
        supprimerBien(bien: IBien): void;
    }
}

declare namespace ICCreditComponents.Navigation {
    import TotemItem = ICCreditComponents.Navigation.TotemV2.ITotemItemV2;
    class TotemV2ItemDirectiveControleur {
        private $document;
        private $timeout;
        private contextService;
        static parametres: ICCommon.Controleur.IControleurParametre;
        ...
        static $inject: string[];
        constructor($document: ng.IDocumentService, $timeout: ng.ITimeoutService, contextService: ICCommon.Services.IProcessusContextService);
        onChevronClicked(item: TotemItem): void;
        needHorizontalDisplay(item: TotemItem): boolean;
        isSubMenuExtand(item: TotemItem): boolean;
        isSelectedLeft(): boolean;
        isSelected(): boolean;
        displayIconExpand(): boolean;
...
declare namespace ICCreditComponents.Navigation {
    class TotemDirectiveControleur {
        ...
        hasBadge(item: ITotemItem): boolean;
        getBadge(item: ITotemItem): number;
...
declare namespace ICCreditComponents.Navigation {
    class TotemChapitreActionsComplementaires {
        ...
        private habilitationService;
        private queryMultiCallService;
        constructor(context: ICBusiness.Models.Common.IContext, etatDossierService: ICBusiness.Services.Dossier.IEtatDossierService, paramApplicatifRemanentService: ICBusiness.Services.ParametrageApplicatif.IParametrageAppicatifRemanentService, controleSyntheseService: Controle.IControleSyntheseService, navigationService: INavigationCreditService, labelService: Services.IlabelService, suiviEditionsService: ICBusiness.Services.SuiviEditions.ISuiviEditionsService, habilitationService: ICBusiness.Services.Habilitation.IHabilitationService, queryMultiCallService: ICCommon.Services.IRetryRequestService);
